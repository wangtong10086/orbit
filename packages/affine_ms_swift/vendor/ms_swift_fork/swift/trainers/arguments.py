...
class TrainArgumentsMixin:
    ...
    def __post_init__(self):
        ...
        if self.gradient_checkpointing:
            if getattr(self, 'ddp_find_unused_parameters', None) is None:
                # HF Trainer falls back to find_unused_parameters=True for wrapped modules such as PEFT models,
                # which is explicitly incompatible with activation checkpointing in distributed training.
                self.ddp_find_unused_parameters = False
                logger.info('Setting args.ddp_find_unused_parameters: False for gradient checkpointing.')
            if getattr(self, 'ddp_broadcast_buffers', None) is None:
                self.ddp_broadcast_buffers = False
                logger.info('Setting args.ddp_broadcast_buffers: False for gradient checkpointing.')
        self._init_liger()
