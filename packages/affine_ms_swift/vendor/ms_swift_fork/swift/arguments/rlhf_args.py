...
class RLHFArguments(TeacherModelArguments, GRPOArguments, PPOArguments, RewardModelArguments, SftArguments):
    ...
    def _check_grpo(self):
        ...
        if self.use_vllm and self.vllm_mode == 'colocate' and self.device_map is None and not is_mp():
            self.device_map = 'cpu'
            logger.info('Setting device_map=cpu for single-process GRPO colocate to avoid GPU OOM during adapter init.')
