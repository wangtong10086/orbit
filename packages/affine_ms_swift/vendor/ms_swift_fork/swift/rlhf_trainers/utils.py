...
def profiling_context(trainer, name: str):
    logger = get_logger()
    stage_name = f'{trainer.__class__.__name__}.{name}'
    logger.info(f'[profiling:start] {stage_name}')
    start_time = time.perf_counter()
    yield
    end_time = time.perf_counter()
    duration = end_time - start_time
    logger.info(f'[profiling:end] {stage_name} took={duration:.6f}s')
...
def safe_gather_variable_tensor(
    tensor: torch.Tensor,
    *,
    stage: str,
    tensor_name: str,
    ...
    """Gather tensors across ranks, using object gather when dim-0 is variable."""
    if logger is None:
        logger = get_logger()
    if tensor.ndim == 0:
    ...
    all_shapes = gather_object([local_shape])
    all_numels = gather_object([local_numel])
    logger.info(
        f'[collective:pre] stage={stage} tensor={tensor_name} '
        f'local_shape={tuple(local_shape)} local_numel={local_numel} '
        f'all_shapes={all_shapes} all_numels={all_numels}'
    )

    if len({tuple(shape) for shape in all_shapes}) == 1:
        return gather(tensor)

    trailing_shapes = {tuple(shape[1:]) for shape in all_shapes}
    if len(trailing_shapes) != 1:
        raise RuntimeError(
            f'Variable-shape gather mismatch for {tensor_name} at {stage}: all_shapes={all_shapes}'
        )

    gathered_tensors = gather_object([tensor.detach().cpu()])
    if not gathered_tensors:
        empty_shape = (0, *next(iter(trailing_shapes)))
        return torch.empty(empty_shape, dtype=tensor.dtype, device=target_device)
    return torch.cat([item.to(target_device) for item in gathered_tensors], dim=0)
