...
from .colocate_driver import ColocateEpisodeDriver, TorchTPGroupCommandExecutor, gather_rank_metadata
...
class RolloutTrainerMixin(RLHFTrainerMixin):
    ...
    def _prepare_vllm(self):
        ...
        self.tp_group = None
        self.tp_group_local_rank = 0
        self.tp_group_src_rank = self.accelerator.process_index
        ...
        if self.vllm_mode == 'server':
            ...
            if self.vllm_tensor_parallel_size > 1:
                ...
                self.tp_group_local_rank = torch.distributed.get_rank(group=self.tp_group)
                self.tp_group_src_rank = self.accelerator.process_index - self.tp_group_local_rank
    ...
    def _infer_single_or_multi_turn(self,
        ...
        if self.multi_turn_scheduler and self.vllm_mode == 'colocate':
            if self.vllm_tensor_parallel_size > 1:
                rollout_outputs = self._colocate_multi_turn_driver(inputs, request_config)
                return self._postprocess_rollout_outputs(inputs, rollout_outputs)
            prepared_requests = [
                resolve_sync(self.multi_turn_scheduler.prepare_request(request, request_config))
                for request in self.inputs2requests(inputs)
            ]
            rollout_outputs = self._colocate_rollout(prepared_requests, request_config)
        else:
            rollout_outputs = self._rollout(inputs, request_config, is_global_inputs)

        if not self.multi_turn_scheduler or self.enable_server_multi_turn:
            return self._postprocess_rollout_outputs(inputs, rollout_outputs)

        return self._colocate_multi_turn_infer(
            inputs,
            prepared_requests or self.inputs2requests(inputs),
            rollout_outputs,
            request_config,
        )

    def _colocate_multi_turn_infer(self, inputs: DataType, requests: List[RolloutInferRequest],
        ...
        accumulators = [RolloutTurnAccumulator() for _ in range(orig_size)]
        ...
        while True:
            ...
            for index, output in zip(index_to_infer, outputs):
                ...
                step_result = resolve_sync(self.multi_turn_scheduler.advance(requests[index], response_choice, current_turn))
                current_logprobs = step_result.get('rollout_logprobs') or self._extract_logprobs_from_choice(response_choice)
                if step_result.get('rollout_infos'):
                    accumulators[index].rollout_infos.update(step_result['rollout_infos'])
                if step_result.get('finished'):
                    accumulators[index].record_final_response(
                        response_choice,
                        is_continuation=is_continuations[index],
                        current_logprobs=current_logprobs,
                    )
                    rollout_outputs[index] = accumulators[index].build_output(
                        response=output.response,
                        messages=requests[index].messages,
                ...
                accumulators[index].record_step_result(
                    step_result,
                    is_continuation=is_continuations[index],
                    current_logprobs=current_logprobs,
                )
    ...
    def _colocate_multi_turn_driver(self, inputs: DataType, request_config: RequestConfig) -> List[RolloutOutput]:
        infer_requests = self.inputs2requests(inputs)
        local_rank_in_group = self.tp_group_local_rank
        local_input_length = len(infer_requests)
        ...
        torch.distributed.all_gather_object(gathered_inputs, infer_requests, group=self.tp_group)
        global_requests = [request for request_list in gathered_inputs for request in request_list]

        local_metadata = {
            'stage': 'colocate_multi_turn_driver',
            'local_rank': local_rank_in_group,
            'local_input_length': local_input_length,
        }
        all_metadata = gather_rank_metadata(local_metadata, group=self.tp_group)
        if local_rank_in_group == 0:
            logger.info(
                '[tp_driver] stage=colocate_multi_turn_driver group_rank=%s all_input_lengths=%s gathered_rank_metadata=%s',
                self.tp_group_src_rank,
                all_input_lengths,
                all_metadata,
            )

        driver = ColocateEpisodeDriver(
            executor=TorchTPGroupCommandExecutor(
                group=self.tp_group,
                src_rank=self.tp_group_src_rank,
                local_rank=local_rank_in_group,
                group_size=self.vllm_tensor_parallel_size,
            ),
            scheduler=self.multi_turn_scheduler,
            infer_fn=self._engine_infer,
            request_config=request_config,
            local_rank_in_group=local_rank_in_group,
            local_input_lengths=all_input_lengths,
            extract_logprobs_fn=self._extract_logprobs_from_choice,
            is_leader=local_rank_in_group == 0,
        )
        return driver.run(global_requests)
    ...
    def _colocate_rollout(self, inputs: Union[DataType, List[RolloutInferRequest]],
                          request_config: RequestConfig) -> List[RolloutOutput]:
        """Perform co-located rollout inference with TransformersEngine or vLLMEngine"""
        infer_requests = inputs if inputs and isinstance(inputs[0], RolloutInferRequest) else self.inputs2requests(inputs)
        if self.vllm_tensor_parallel_size > 1:
            local_rank_in_group = self.tp_group_local_rank
            local_input_length = len(infer_requests)
            all_input_lengths = [None] * self.vllm_tensor_parallel_size
            ...
            gathered_inputs = [None for _ in range(self.vllm_tensor_parallel_size)]
            torch.distributed.all_gather_object(gathered_inputs, infer_requests, group=self.tp_group)
            infer_requests = [p for sublist in gathered_inputs for p in sublist]
            if local_rank_in_group == 0:
                logger.info(
                    f'[tp_rollout] stage=colocate_rollout leader_rank={self.tp_group_src_rank} '
                    f'all_input_lengths={all_input_lengths} total_requests={len(infer_requests)}'
                )
            outputs = self._engine_infer(infer_requests=infer_requests, request_config=request_config)
            if local_rank_in_group == 0:
                logger.info(
                    f'[tp_rollout] stage=colocate_rollout leader_rank={self.tp_group_src_rank} '
        ...
        outputs: List[RolloutOutput] = self._engine_infer(infer_requests=infer_requests, request_config=request_config)
    ...
    def _engine_infer(
        ...
        *,
        ...
        For colocate mode with TP>1, all ranks must call engine.infer() with the
        same requests so that every rank's vLLM scheduler sees the same work and
        enters execute_model() together.  Leader-only request submission causes a
        deadlock because the non-leader scheduler is empty and skips
        execute_model() while the leader's model executor waits for all TP ranks.
        """
        with profiling_context(self, 'generate'), self._disable_sp_context():
            if self.vllm_mode == 'server':
                ...
                if (self.vllm_mode == 'colocate'
                        ...
                        and request_config is not None
                        and request_config.seed is None):
                    # external_launcher requires every TP rank to produce
                    # identical scheduler decisions.  With temperature > 0
                    # and no per-request seed, each rank samples different
                    # tokens, causing _ALLGATHER_BASE size mismatches on
                    # later steps.  Derive a deterministic seed from the
                    # engine request counter so all ranks stay in lock-step
                    # while still varying across inference calls.
                    request_config = deepcopy(request_config)
                    request_config.seed = 42 + self.engine._request_count
    ...
    def _engine_infer_tp_group(
        self,
        infer_requests: List[RolloutInferRequest],
        request_config: Optional[RequestConfig],
        *,
        use_tqdm: Optional[bool] = False,
    ) -> List[Union[RolloutOutput, ChatCompletionResponse]]:
        """Run colocate vLLM inference with leader-only request submission and all-rank stepping.

        This mirrors the control model used by external rollout runtimes:
        one rank owns request submission and output collection, while every TP rank
        participates in the engine stepping loop.
        """
        assert self.vllm_mode == 'colocate'
        assert self.vllm_tensor_parallel_size > 1
        assert self.tp_group is not None

        tp_local_rank = self.tp_group_local_rank
        is_leader = tp_local_rank == 0
        engine_wrapper = self.engine
        llm_engine = engine_wrapper.engine
        ...
        if is_leader:
            ...
            engine_wrapper.template.set_mode('vllm')
            batched_inputs, error_list = engine_wrapper._batch_encode(
                infer_requests, strict=getattr(engine_wrapper, 'strict', True))
            request_lengths = [len(inputs.get('input_ids', [])) for inputs in batched_inputs]
            logger.info(
                '[tp_infer] stage=submit rank=%s batch_size=%s request_lengths=%s',
                self.tp_group_src_rank,
                len(batched_inputs),
                request_lengths,
            )
            for i, inputs in enumerate(batched_inputs):
                request_id = str(engine_wrapper._request_count)
                request_id_list.append(request_id)
                engine_wrapper._request_count += 1
                _request_config = deepcopy(request_config)
                engine_wrapper.set_default_max_tokens(_request_config, inputs)
                generation_config = engine_wrapper._prepare_generation_config(_request_config)
                if generation_config.seed is not None:
                    generation_config.seed += i
                engine_wrapper._add_stop_words(generation_config, _request_config)
                engine_wrapper._add_request(inputs, generation_config, request_id)

        if dist.is_available() and dist.is_initialized():
            payload = [request_lengths]
            torch.distributed.broadcast_object_list(payload, src=self.tp_group_src_rank, group=self.tp_group)
            if not is_leader:
                logger.info(
                    '[tp_infer] stage=wait rank=%s batch_size=%s request_lengths=%s',
        ...
        outputs = {}
        while True:
            has_unfinished = llm_engine.has_unfinished_requests() if is_leader else None
            status_payload = [has_unfinished]
            torch.distributed.broadcast_object_list(status_payload, src=self.tp_group_src_rank, group=self.tp_group)
            if not status_payload[0]:
                break
        ...
        result_payload: List[Optional[List[Union[RolloutOutput, ChatCompletionResponse]]]] = [None]
        if is_leader:
            ordered_outputs = [outputs[request_id] for request_id in request_id_list]
            responses = [
                engine_wrapper._create_chat_completion_response(result, inputs, request_config, request_id)
                for request_id, inputs, result in zip(request_id_list, batched_inputs, ordered_outputs)
            ]
            result_payload[0] = engine_wrapper._add_error_list(responses, error_list)
        torch.distributed.broadcast_object_list(result_payload, src=self.tp_group_src_rank, group=self.tp_group)
        result = result_payload[0]
        if result is None:
            raise RuntimeError('TP colocate inference returned no result payload')
        return result
        ...
        if args.multi_turn_scheduler:
            ...
            if isinstance(args.multi_turn_scheduler, str):
                ...
                multi_turn_scheduler = multi_turns[args.multi_turn_scheduler](
                    ...
                    context_manager=getattr(args, 'context_manager', None),
                )
    ...
    def multi_turn_completion_length_context(self):
        ...
        def set_default_max_tokens(_self, request_config: RequestConfig, inputs: Dict[str, Any]) -> None:
            ...
            if not hasattr(_self, 'set_grpo_max_model_len'):
                ...
                nonlocal patched_max_model_len
                patched_max_model_len = True
            else:
        ...
        try:
            ...
            if patched_max_model_len and hasattr(self.engine, 'set_grpo_max_model_len'):
