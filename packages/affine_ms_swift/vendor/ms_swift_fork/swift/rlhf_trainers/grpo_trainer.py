...
class GRPOTrainer(RolloutTrainerMixin, SwiftMixin, HFGRPOTrainer):
    ...
    def _score_completions(self, inputs: DataType) -> torch.Tensor:
        ...
        if not self.dynamic_num_samples:
            total_rewards_per_func = safe_gather_variable_tensor(
                local_rewards_per_func,
                stage='score_completions',
                tensor_name='local_rewards_per_func',
                logger=logger,
                device=self.accelerator.device,
            )
        else:
    ...
    def _compute_advantages(self, inputs: DataType, rewards_per_func: torch.Tensor,
        ...
        if self.kl_in_reward and self.beta != 0.0:
            ...
            kl = safe_gather_variable_tensor(
                kl,
                stage='compute_advantages',
                tensor_name='kl_in_reward',
                logger=logger,
                device=self.accelerator.device,
            )
            mode = 'train' if self.model.training else 'eval'
    ...
    def _compute_loss_and_metrics(self, model, inputs):
        ...
        if self.compute_entropy:
            ...
            if self.args.log_entropy:
                ...
                global_per_completion_entropies_mean = safe_gather_variable_tensor(
                    per_completion_entropies_mean,
                    stage='compute_loss',
                    tensor_name='per_completion_entropies_mean',
                    logger=logger,
                    device=self.accelerator.device,
                )
                entropy_metrics = {
