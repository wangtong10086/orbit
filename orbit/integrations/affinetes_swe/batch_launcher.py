...
def _sample_remote_metrics(
    *,
    student_ssh_host: str,
    student_ssh_port: int,
    student_ssh_user: str,
    student_log_path: str,
) -> dict[str, Any]:
...
def launch_clean_eval_batch(
    *,
    ...
    prewarm_path = Path(image_prewarm_json).resolve()
    ...
    runs = {str(item["task_id"]): TaskRun(task_id=str(item["task_id"])) for item in tasks}
    ...
    def _record_state() -> None:
        payload = {
            ...
            "image_prewarm_json": str(prewarm_path),
            "output_dir": str(root),
    ...
    while True:
        ...
        active_total = sum(1 for run in runs.values() if run.status in {"bootstrapping", "running"})
        active_bootstraps = sum(1 for run in runs.values() if run.status == "bootstrapping")
        pending = [run for run in runs.values() if run.status == "pending"]
        while pending and active_total < max_live_rollouts and active_bootstraps < bootstrap_concurrency:
        ...
        if now - last_metrics >= metrics_interval_secs:
            ...
            metrics = {
                ...
                "started": sum(1 for run in runs.values() if run.started or run.status in {"completed", "failed_infra", "failed_model"}),
                "completed": sum(1 for run in runs.values() if run.status == "completed"),
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--image-prewarm-json", required=True)
