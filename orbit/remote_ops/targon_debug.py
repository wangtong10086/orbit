...
def _game_debug_include_paths(config: OrbitConfig) -> list[tuple[Path, str]]:
    root = config.project_root
    return [
        (root / "orbit" / "__init__.py", "orbit/__init__.py"),
        (root / "orbit" / "data" / "__init__.py", "orbit/data/__init__.py"),
        (root / "orbit" / "data" / "game_gen.py", "orbit/data/game_gen.py"),
        (
            root / "orbit" / "data" / "game_trajectory_generators.py",
            "orbit/data/game_trajectory_generators.py",
        ),
        (root / "orbit" / "data" / "game_generators", "orbit/data/game_generators"),
        (root / "orbit" / "foundation" / "__init__.py", "orbit/foundation/__init__.py"),
        (root / "orbit" / "foundation" / "schema.py", "orbit/foundation/schema.py"),
...
def _create_game_debug_snapshot(config: OrbitConfig, output_path: Path) -> Path:
...
def _upload_game_debug_snapshot(config: OrbitConfig, local_path: Path, repo_id: str, path_in_repo: str) -> dict:
    try:
...
def _build_game_smoke_script(
    *,
    ...
    parts = [
        ...
        "tar -xzf game_debug.tar.gz -C /tmp/personal-project",
        "cd /tmp/personal-project",
        "export PYTHONPATH=/tmp/personal-project:${PYTHONPATH:-}",
