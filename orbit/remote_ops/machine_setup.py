    ...
    async def _run():
        ...
        if rc != 0:
            ...
            if err:
                click.echo(err[:500])
            return
        click.echo("Next: orbit remote machine clone-eval <source_machine>")
...
def bootstrap(ctx, training_only, check):
    ...
    config = ctx.obj["config"]
    backend, inst = get_rental(config, _machine_selector(ctx))
    setup_dir = config.project_root / "orbit" / "setup"
...
@click.argument("tag", default="wangtong123/orbit:latest")
