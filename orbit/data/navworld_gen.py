...
from orbit.data.amap_client import AMapClient, execute_tool
from orbit.data.navworld_plans import TOOL_PLANS
from orbit.data.navworld_prompts import (
    ALL_PROBLEM_TYPES,
    PHASE1_TYPES,
    SYSTEM_PROMPT,
    TOOLS_SCHEMA,
...
async def generate_conversation(
    ...
    llm_user_content = (
        ...
        f"【评分规则——严格遵守】\n"
        f"评分系统会检查：每个关键词（如'航班''价格''景点'）旁边500字符内是否有对应的工具真实数据。\n"
        f"因此，每提到一个主题时，必须在同一段落内紧跟引用工具数据。不要把数据和分析分开写。\n\n"
        f"【必须满足的要求】\n"
        f"1. 每提到航班/飞机/机票时，同一段落内必须紧跟航班号+价格XX元+时间HH:MM（至少3个航班）\n"
        f"2. 每提到火车/高铁/车次时，同一段落内必须紧跟车次号+价格XX元+时间HH:MM（至少3个车次）\n"
        f"3. 每提到景点/游览/参观时，同一段落内必须引用工具返回的景区/公园/博物馆名称（至少3个）\n"
        f"4. 每提到餐/吃/美食时，同一段落内必须引用工具返回的餐厅/饭店名称（至少2个）\n"
        f"5. 每提到住宿/酒店/宾馆时，同一段落内必须引用工具返回的酒店/宾馆名称（至少2个）\n"
        f"6. 每提到预算/费用/花费时，同一段落内必须有具体金额XX元（至少3个）\n"
        f"7. 每提到天气/气温/穿衣时，同一段落内必须引用气温XX度、天气（晴/阴/雨）、风力\n"
        f"8. 每提到交通/距离/路线时，同一段落内必须引用XX公里/米、约XX分钟\n"
        f"9. 使用推理词（因为/建议/推荐/考虑到/对比/相比/优先/适合）解释每个选择\n"
        f"10. 方案至少2000字\n\n"
        f"【必须包含的章节——每章紧密结合数据】\n"
        f"11. **交通方案对比**：列出至少3个航班号/车次号，每个紧跟出发时间HH:MM、到达时间HH:MM、价格XX元。用表格对比。\n"
        f"12. **景点游览安排**：每个景点名称后紧跟地址、游览建议时间、门票价格\n"
        f"13. **餐饮推荐**：每个餐厅名称后紧跟地址、人均XX元、推荐菜品\n"
        f"14. **住宿建议**：每个酒店名称后紧跟地址、价格区间、距离XX公里\n"
        f"15. **交通路线**：引用距离XX公里和时间约XX分钟，用先/然后/接着描述\n"
        f"16. **天气与穿衣**：引用气温、天气、风力，给出建议\n"
        f"17. **预算明细**：分项列出交通XX元+住宿XX元+餐饮XX元+门票XX元=总计XX元\n"
