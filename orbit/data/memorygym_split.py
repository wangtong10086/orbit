...
def split_dataset(
    *,
    ...
    all_samples = []
    trajectories = [json.loads(line) for line in input_file.open()]
    ...
    for event_type, count in sorted(type_counts.items()):
        print(f"  {event_type}: {count} ({count/len(all_samples)*100:.1f}%)")

    rng = Random(shuffle_seed)
    if target > 0 or balance:
        effective_target = target if target > 0 else len(all_samples)
        all_samples = balance_samples(all_samples, effective_target, rng)
    ...
    for sample in all_samples:
        event_type = sample["event_type"]
        type_counts[event_type] = type_counts.get(event_type, 0) + 1
        token_lengths.append(_est_tokens(sample["messages"]))
    ...
    if token_lengths:
        ...
        print(f"  > 32K: {sum(1 for length in token_lengths if length > 32000)}")
    print(f"\n  Output: {output_file}")
    return {
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Split MemoryGym trajectories into per-event samples")
    parser.add_argument("-i", "--input", required=True, help="Input JSONL (full trajectories)")
    parser.add_argument("-o", "--output", required=True, help="Output JSONL (per-event samples)")
    parser.add_argument("--target", type=int, default=0, help="Target sample count (0 = keep all)")
    parser.add_argument("--balance", action="store_true", help="Balance event type distribution")
