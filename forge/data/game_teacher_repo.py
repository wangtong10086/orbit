...
from forge.data.game_generators.policy_generators import load_policy_snapshot
...
def _resolve_repo(repo_id: str = "") -> str:
    return repo_id or os.environ.get("HF_GAME_TEACHER_REPO", "") or ForgeConfig.load().hf_game_teacher_repo


def _resolve_token(token: str = "") -> str:
    return token or os.environ.get("HF_TOKEN", "") or ForgeConfig.load().hf_token
...
forge data game-upload-teacher --game leduc_poker --repo {repo_id}
```
...
```bash
forge data game-build-expert-dataset --game leduc_poker --samples 128
forge data game-train-policy-model --game leduc_poker --dataset artifacts/game_expert_datasets/leduc_poker/expert_dataset.npz
```
"""
