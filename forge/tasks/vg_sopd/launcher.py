...
from forge.core.contracts.execution import ArtifactManifest, JobKind, RunState
from forge.core.contracts.tasks import TaskSubmission
from forge.core.contracts.templates import ExecutionOverrides
from forge.core.experiments.models import Experiment, TrainingLifecycleState
...
from forge.tasks.vg_sopd.data_utils import write_json
...
def _resolve_target_name(execution: StageExecutionSpec, forge_config: ForgeConfig, cache: dict[str, dict]) -> tuple[str, dict | None]:
...
def _stage_overrides(execution: StageExecutionSpec, forge_config: ForgeConfig, cache: dict[str, dict]) -> tuple[ExecutionOverrides, dict | None]:
    target_name, provision_payload = _resolve_target_name(execution, forge_config, cache)
    return (
        ExecutionOverrides(
...
def _submit_stage(
    ...
    *,
    ...
    overrides, provision_payload = _stage_overrides(execution, forge_config, provision_cache)
    plane.submit_task(
        TaskSubmission(
            experiment_id=experiment_id,
            task_type=task_type,
...
def launch_vg_sopd_from_config(
    ...
    *,
    ...
    frontier_source_path = _resolve_dataset_path(launch_config.frontier_task_source, forge_config)
    cold_start_source_path = _resolve_dataset_path(launch_config.cold_start.dataset, forge_config) if launch_config.cold_start.enabled and launch_config.cold_start.dataset is not None else ""
...
def launch_vg_sopd_from_path(
    ...
    *,
    ...
    return launch_vg_sopd_from_config(plane, launch_config, forge_config=forge_config, config_path=config_path)
