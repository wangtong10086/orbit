...
@data.command(name="navworld-enhance")
@click.option("--score-only", is_flag=True, help="Just score and report, don't rewrite")
@click.option("--rewrite-n", default=0, type=int, help="Rewrite bottom N entries (0=all below target)")
@click.option("--target-score", default=15, type=int, help="Minimum acceptable score out of 50")
@click.option("--score-sample", default=200, type=int, help="Number of entries to score for triage")
@click.option("--concurrency", default=5, type=int, help="Parallel API calls for rewriting")
@click.option("--dry-run", is_flag=True, help="Score + rewrite but don't replace canonical")
@click.pass_context
def navworld_enhance(ctx, score_only, rewrite_n, target_score, score_sample, concurrency, dry_run):
    """Enhance NAVWORLD data quality using Claude API (score + two-pass rewrite).

    Scores entries on 5 eval dimensions, rewrites low-quality ones with a
    Sonnet-generate -> Haiku-critique -> Sonnet-fix pipeline.

    Examples:
      forge data navworld-enhance --score-only                    # Just score 200 entries
      forge data navworld-enhance --rewrite-n 20 --dry-run        # Rewrite 20, don't save
      forge data navworld-enhance --target-score 20               # Rewrite everything below 20/50
      forge data navworld-enhance --rewrite-n 50 --concurrency 3  # Rewrite 50, 3 parallel
    """
    from forge.data.navworld_enhance import enhance_batch, get_claude_client, CANONICAL_PATH

    client = get_claude_client()
    canonical = CANONICAL_PATH
    if not canonical.exists():
        raise click.ClickException(f"Canonical file not found: {canonical}")

    max_rewrites = rewrite_n if rewrite_n > 0 else 9999

    click.echo(f"NAVWORLD Enhancement Pipeline")
    click.echo(f"  Canonical: {canonical}")
    click.echo(f"  Target score: {target_score}/50")
    click.echo(f"  Score sample: {score_sample}")
    if score_only:
        click.echo(f"  Mode: score-only")
    elif dry_run:
        click.echo(f"  Mode: dry-run (rewrite up to {max_rewrites})")
    else:
        click.echo(f"  Mode: full (rewrite up to {max_rewrites})")
    click.echo()

    result = asyncio.run(enhance_batch(
    ...
    click.echo(f"  Entries scored: {result.get('scored', 0)}/{result.get('total_entries', 0)}")
    click.echo(f"  Average score: {result.get('avg_score', 0)}/50")
    click.echo(f"  Below target ({target_score}): {result.get('below_target', 0)}")

    dim_avgs = result.get("dimension_avgs", {})
    if dim_avgs:
        click.echo(f"  Dimension averages:")
        for d, v in dim_avgs.items():
            click.echo(f"    {d}: {v}/10")

    if not score_only:
        click.echo(f"  Rewritten: {result.get('rewritten', 0)}")
        click.echo(f"  Failed: {result.get('failed', 0)}")
        if result.get('verified_sample'):
            click.echo(f"  Verified improved: {result.get('verified_improved', 0)}/{result.get('verified_sample', 0)}")
        if result.get('dry_run'):
            click.echo(f"  [DRY RUN] Would replace: {result.get('would_replace', 0)}")
        elif result.get('replaced'):
            click.echo(f"  Replaced in canonical: {result['replaced']}")

    click.echo(f"\n{result.get('cost', 'No cost data')}")
