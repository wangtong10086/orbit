...
upload_forge() {
    ...
    for f in $SCRIPTS; do
        .venv/bin/python3 -m orbit rental -m "$target" upload "$f" "/root/project/$f" 2>&1 | grep -v protocol
    done
    .venv/bin/python3 -m orbit rental -m "$target" exec "rm -rf /root/project/scripts/game/__pycache__; mkdir -p /root/project/data/v11" 2>&1 | tail -1
}
...
start_forge() {
    ...
    .venv/bin/python3 -m orbit rental -m "$target" exec \
...
for m in m1 m2 m3; do
    .venv/bin/python3 -m orbit rental -m "$m" exec "pkill -f generate_v11 2>/dev/null; pkill -f orchestrate_v11 2>/dev/null; echo '$m clean'" 2>&1 | tail -1
done
...
echo "Monitor: orbit rental -m [m1|m2|m3] exec 'tail -30 /tmp/v11_orchestrator.log'"
echo "Check:   orbit rental -m [m1|m2|m3] exec 'wc -l /root/project/data/v11/v11_*.jsonl'"
