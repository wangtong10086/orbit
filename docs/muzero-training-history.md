...
## v7：算法正确性修复（2026-04-05）

### 审计发现

在专家审计后发现多个关键算法正确性问题，v4-v6 所有版本均受影响：

| Bug | 严重程度 | 说明 |
|-----|----------|------|
| **策略目标使用 visit counts** | 关键 | 代码沿用 AlphaZero 式 visit-count 归一化作为训练目标，而非 Gumbel MuZero 的 improved policy |
| **Dirichlet 根噪声未关闭** | 关键 | Gumbel MuZero 使用 Gumbel 噪声而非 Dirichlet，但代码保留了 Dirichlet |
| **Reward loss 非零** | 高 | 棋盘游戏无中间奖励，但 reward loss 以 0.05 权重参与梯度 |
| **Dynamics 无梯度缩放** | 高 | MuZero 论文 Appendix G 要求 dynamics 入口 0.5× 梯度缩放，代码缺失 |
| **Latent consistency 原始 MSE** | 中 | 无 stop-gradient 的 MSE 可能干扰 dynamics 学习 |
| **unroll_steps 配置/实际不一致** | 中 | 配置写 `unroll_steps: 3`，实际 learner 只做 K=1 |

### v7 修复内容

**1. Gumbel Improved Policy Target**（`batched_search.py`）

新增 `_gumbel_improved_policy()` 方法：
```
π_improved(a) = softmax(log(prior(a)) + atanh(completedQ(a)))
```
其中 `completedQ(a) = Q(a) if N(a)>0 else root_value`。这是 Gumbel MuZero 论文的标准公式，用 action values 而非 visit counts 构造改进策略。

**2. 条件化 Dirichlet 噪声**（`batched_search.py`）

当 `root_dirichlet_epsilon ≤ 0` 时跳过噪声注入。v7 配置 `root_dirichlet_epsilon: 0.0`。

**3. Dynamics 梯度缩放**（`board_muzero.py`）

在 `recurrent_inference()` 入口添加 0.5× 梯度缩放：
```python
scaled_latent = latent * 0.5 + latent.detach() * 0.5
```

**4. 可配置 Loss 权重**（`learner.py` + `gpu_coordinator.py`）

新增 `loss_weights:` YAML 配置块，支持独立设置 `reward`, `latent`, `recurrent_policy`, `recurrent_value` 权重。v7 配置：
- `reward: 0.0`（棋盘游戏无中间奖励）
- `latent: 0.0`（消融：关闭 MSE latent consistency）
- `recurrent_policy: 0.5`, `recurrent_value: 0.5`

### v7 配置关键参数

| 参数 | Othello | Clobber |
|------|---------|---------|
| init checkpoint | v5 best (step 170k, WR 46%) | v5 best (step 430k, WR 24.5%) |
| lr_online | 1e-4 | 1e-4 |
| root_dirichlet_epsilon | 0.0 | 0.0 |
| teacher_kl_weight | 0.0 | 0.0 |
| reward loss | 0.0 | 0.0 |
| latent loss | 0.0 | 0.0 |
| train_num_simulations | 64 | 64 |
| unroll_steps | 1 | 1 |

### v6 终止数据

v6 因修复后停止（step ~170k）：
- Othello: WR=29.5%（eval_step_130k，低于 v5 峰值 46%）
- Clobber: WR=14%（eval_step_170k，低于 v5 峰值 24.5%）

v6 退步的可能原因：
1. 在 v5 禁用 gated_snapshot 后，从已稳定的 v5 模型继续训练，但 v5 本身因 visit-count 策略目标已达到局部最优
2. LR 和其他超参数在 v5 调整后可能偏保守

### v7 预期

v7 是首次使用正确 Gumbel improved policy 目标的训练。从 v5 best checkpoint 出发，策略头会在新目标下重新校准。预期：
- 短期（<30k steps）：可能出现 WR 下降，因为策略目标从 visit-count 切换到 improved policy
- 中期（50k-200k steps）：应超越 v5 峰值，因为改进策略目标理论上收敛更快
- 关注指标：policy loss 下降速率、WR 变化趋势、value loss 稳定性

### 待实施改进

以下改进已审计确认有价值，但延后到 v7 稳定后实施：

1. **Reanalyse 主路径**：用最新模型重新分析 replay buffer 中的旧轨迹
2. **Clobber 管线审计**：action mask、flip augmentation、value sign 单元测试
3. **State encoding 消融**：改为绝对黑白 + 当前玩家平面
4. **K=5 unroll**：需修改 selfplay 数据管线存储 K+1 连续状态
5. **AlphaZero 对照实验**：用精确模拟器 MCTS 作为诊断基线

---

*文档最后更新：2026-04-05，v7 启动（commit 12d9620）*
